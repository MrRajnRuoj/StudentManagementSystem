...
void loadDatabase(Data &data)
{
	...
	while (!dataUser.eof())
	{
		...
		getline(dataUser, cur.username, ',');
		getline(dataUser, cur.fullName, ',');
		getline(dataUser, cur.email, ',');
		getline(dataUser, cur.mobilePhone, ',');
		dataUser >> cur.type;
		dataUser.ignore();
		getline(dataUser, cur.password, ',');
		getline(dataUser, cur.classOfSt);
		addTail(data.lUser, cur);
	}
	dataUser.close();
